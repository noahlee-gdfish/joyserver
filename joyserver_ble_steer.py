...
def Scale(joynum, x, y):
    ...
    if joynum == 1:
        ...
        if new_x%(roundup*2) < roundup:
            new_x = new_x - (new_x%(roundup*2))
        else:
            new_x = new_x + ((roundup*2)-(new_x%(roundup*2)))

        if new_y%(roundup*2) < roundup:
            new_y = new_y - (new_y%(roundup*2))
        else:
            new_y = new_y + ((roundup*2)-(new_y%(roundup*2)))
...
def GetSteer():
    ...
    SCALE_RATIO = SCALE_STEER_VALUE/JOY_CENTER

    vrx_pos, vry_pos = ReadJoy(0)

    if vrx_pos < JOY_CENTER:
        direction = "l"
        speed = int((JOY_CENTER - vrx_pos) * SCALE_RATIO)
        if speed % 2:
            speed += 1
    elif vrx_pos > JOY_CENTER:
        direction = "r"
        speed = int((vrx_pos - JOY_CENTER) * SCALE_RATIO)
        if speed % 2:
            speed += 1
    else:
        direction = "l"
...
def main_test():
    ...
    while True:
        try:
            speed, direction = GetSpeed()
            msg = "{0}{1}".format(speed, direction)
            if msg != lastSpeedMsg:
                print(msg)
                lastSpeedMsg = msg

            speed, direction = GetSteer()
            msg = "{0}{1}".format(speed, direction)
            if msg != lastSteerMsg:
                print(msg)
                lastSteerMsg = msg
